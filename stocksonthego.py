...
def wyznacz_rank(ticker, inputfile=None, z_pliku=True, df=None, wczoraj = True):
    ...
    try:
        ...
        return [
            ...
            "=(WartoscPortfela*Zmiennosc)/indirect(address(row(),column()+6))",          # ile sztuk akcji kupić || (zmiennosc * wartosc portfela) : atr20
...
def pobierz_yahoo_bulk(test=False, wczoraj = True):
    ...
    column_names = ["Ticker","Name", "Close","IleAkcji", "WlkPozycji", "Date", "Low50", "Stop?(C<LL50)", "ZaIle", "ATR20", "Rank", "Zacks"]
    ...
    gc = pygsheets.authorize( client_secret='cred.json')

    # open the google spreadsheet
    sh = gc.open('stocks on the go' )
...
def get_sp400_tickers():
    ...
    df = table[0]
    return df['Ticker symbol'].values
